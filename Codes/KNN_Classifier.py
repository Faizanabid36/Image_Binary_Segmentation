...
# Read image
image = cv2.imread("/home/fadlo/My_Projects/Machine_Vision/Projects/Project_1/project01/TestingImages/41033.jpg")
image_R = image[:,:,0]
image_G = image[:,:,1]
image_B = image[:,:,2]
...
min_k_distances = min_distance_index_3D[:,:,0:3]       #Assuming K=3 


x = np.zeros((min_k_distances.shape[0], min_k_distances.shape[1]))


for k in range(min_k_distances.shape[2]):
    for i in range(min_k_distances.shape[0]):
        for j in range(min_k_distances.shape[1]):
            if min_k_distances[i,j,k] >= 2:
                x[i,j] = x[i,j] + 1
...
cv2.imshow("Segmented Image using KNN - K=3", image)
cv2.waitKey(0)
